"""
Physics engines for top-down or platformers. This is my version to play around with.
"""
...
def _move_sprite(
    moving_sprite: Sprite, walls: List[SpriteList], ramp_up: bool
) -> List[Sprite]:
    """
    Move a sprite and resolve collisions. Returns a list of all sprites that were collided with. This list will be empty if no collisions occurred.
    """
...
class PhysicsEnginePlatformer:
    ...
    **Note:** This engine will automatically move any Sprites sent to the ``platforms``
    ...
    def __init__(
        self,
        player_sprites: Union[SpriteList, Iterable[SpriteList]],
        ...
        self.player_sprites: List[SpriteList]
        ...
        if isinstance(player_sprites, SpriteList):
            ...
            self.player_sprites = list(player_sprites)

        # self.player_sprites: Sprite = player_sprites
    ...
    def is_on_ladder(self):
        ...
        if self.ladders:
            hit_list = check_for_collision_with_lists(self.player_sprites, self.ladders)
            if len(hit_list) > 0:
                return True
        return False
    ...
    def can_jump(self, y_distance: float = 5) -> bool:
        ...
        self.player_sprites.center_y -= y_distance

        # Check for wall hit
        hit_list = check_for_collision_with_lists(
            self.player_sprites, self.walls + self.platforms
        )
    ...
    def jump(self, velocity: int):
        ...
        self.player_sprites.change_y = velocity
    ...
    def update(self):
        ...
        if not self.is_on_ladder():
            self.player_sprites.change_y -= self.gravity_constant
        ...
        complete_hit_list = _move_sprite(
            self.player_sprites, self.walls + self.platforms, ramp_up=True
        )
